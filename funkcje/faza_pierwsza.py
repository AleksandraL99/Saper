...
def pierwszy_klik(window, bomby, dane, wspolrzedna_1, wspolrzedna_2):
    funkcje.faza_druga.faza_druga(window, bomby, dane, wspolrzedna_1, wspolrzedna_2)
...
def faza_pierwsza(window, dane, bomby):
    ...
    for i in elementy:  # niszczymy poprzenie okno
        i.destroy()
    ...
    # przycisk resetu
    reset = tkinter.Button(frame, bg=funkcje.stale.SZARY_JASNY,
                           width=WYMIAR_RESET, heigh=WYMIAR_RESET, image=photo)
    reset.place(x=3, y=8)

    board = tkinter.Frame(frame, bg=funkcje.stale.SZARY_CIEMNY,
    ...
    for i in range(wysokosc):
        for j in range(szerokosc):
            # przyciski
            strzal = tkinter.Button(board, bg=funkcje.stale.SZARY_JASNY, text="     ",
